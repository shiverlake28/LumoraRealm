...
function Login(){
    function HandleSubmit(e:React.FormEvent<HTMLFormElement>){
        ...
        console.log("2 - Password value:", Pword);
        SetLoading(true)
    }
    ...
    return(
        <div className="w-full min-h-screen font-space flex flex-col  justify-start items-center  pt-8 bg-linear-to-b from-[rgb(1,28,30)] via-[rgb(4,54,56)] to-[rgb(2,34,36)]">{/*entire bg*/}
            ...
            <div className="w-100 h-auto  pt-1 flex flex-col items-center justify-start gap-3 bg-linear-to-b from-[rgb(8,70,72)] via-[rgb(11,82,84)] to-[rgb(14,96,98)] border-3 border-stone-400 hover:border-gray-400 shadow-xl shadow-gray-400/50 hover:shadow-gray-400 rounded-3xl">
                ...
                {/* email */}
                <form action=" " id="LoginForm"  onSubmit={HandleSubmit} className="w-full pl-5 pr-5 flex flex-col gap-3 h-auto   text-[rgb(248,242,236)]/90 ">
                    <label className="text-lg  text-[rgb(248,242,236)]">Email</label>
                    <input type="email" value={Email} placeholder="e.g:you@gmail.com" onChange={(e)=>{SetEmail(e.target.value); SetEmailError("");}} className=" pl-2 text-black  w-full h-10 border border-black/90 rounded-md bg-[rgb(177,178,181)] placeholder:text-black/40 hover:shadow-sm hover:shadow-black"/>
                    ...
                    <input type={ShowPassword?"text":"password"} onChange={(e)=>{SetPword(e.target.value);SetPwordError("");}} placeholder="Password"  className="w-full pl-2 text-black h-10 border border-black/90 rounded-md bg-[rgb(177,178,181)] placeholder:text-black/40 hover:shadow-sm hover:shadow-black"/>
                ...
                {/* Buttons */}
                <button form="LoginForm" disabled={Loading} className={`w-50 h-12  border-2 rounded-3xl bg-[rgba(30,41,59,0.81)] text-[rgb(244,230,236)]/80 ${Loading? 'opacity-60 cursor-not-allowed': "opacity-100 cursor-pointer"}`} >{Loading? "Loading..." :"Login"}</button>
                ...
                <p className="text-[12px] w-full pl-6 pb-5 text-[rgb(220,220,220)]">Don't have an account? <Link to="/SignUps"  className="underline text-[rgb(248,246,236)]/90 ">SignUp</Link></p>
            </div>
        </div>
    )
}
